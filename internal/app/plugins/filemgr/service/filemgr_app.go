...
import (
	...
	"go-admin/internal/app/plugins/filemgr/models"
...
func (e *FilemgrApp) Insert(c *dto.FilemgrAppInsertReq) (int64, int, error) {
	...
	if c.Platform == "" {
		return 0, clang.AppPlatformEmptyCode, lang.MsgErr(clang.AppPlatformEmptyCode, e.Lang)
	}
	if c.Version == "" {
		return 0, clang.AppVersionEmptyCode, lang.MsgErr(clang.AppVersionEmptyCode, e.Lang)
	}
	if c.DownloadType == "" {
		return 0, clang.AppDownloadTypeEmptyCode, lang.MsgErr(clang.AppDownloadTypeEmptyCode, e.Lang)
	}
	if c.DownloadType == constant.AppDownloadTypeOss || c.DownloadType == constant.AppDownloadTypeLocal {
		if c.AppType == "" {
			return 0, clang.AppTypeCode, lang.MsgErr(clang.AppTypeCode, e.Lang)
		}
		if c.LocalAddress == "" {
			return 0, clang.AppUploadCode, lang.MsgErr(clang.AppUploadCode, e.Lang)
		}
	}
	if c.Remark == "" {
		return 0, clang.AppRemarkCode, lang.MsgErr(clang.AppRemarkCode, e.Lang)
	}
	...
	if count > 0 {
		return 0, clang.AppExistCode, lang.MsgErr(clang.AppExistCode, e.Lang)
	}
	...
	if c.DownloadType == constant.AppDownloadTypeOss {
		...
		if err != nil {
			return 0, clang.AppOssUploadLogCode, lang.MsgLogErrf(e.Log, e.Lang, clang.AppUploadCode, clang.AppOssUploadLogCode, err)
		}
	}
	if c.DownloadType == constant.AppDownloadTypeLocal {
		if c.LocalRootUrl == "" {
			return 0, clang.AppLocalUrlEmptyCode, lang.MsgErr(clang.AppLocalUrlEmptyCode, e.Lang)
		}
...
func (e *FilemgrApp) Delete(ids []int64, p *middleware.DataPermission) (int, error) {
	...
	for _, id := range ids {
		...
		if respCode != clang.DataNotFoundCode && err != nil {
			return respCode, err
		}
