...
func (e *SysMenu) Get(id int64, p *middleware.DataPermission) (*models.SysMenu, int, error) {
	if id <= 0 {
		//id<=0,表示为顶级根菜单
		return &models.SysMenu{Id: 0, ParentIds: ""}, clang.SuccessCode, nil
	}
	data := &models.SysMenu{}
	err := e.Orm.Scopes(
...
func (e *SysMenu) Insert(c *dto.SysMenuInsertReq) (int64, int, error) {
	if c.ParentId < 0 {
		return 0, clang.SysMenuParentIdEmptyCode, lang.MsgErr(clang.SysMenuParentIdEmptyCode, e.Lang)
	}
	if c.Title == "" {
		return 0, clang.SysMenuTitleEmptyCode, lang.MsgErr(clang.SysMenuTitleEmptyCode, e.Lang)
	}
	if c.MenuType == "" {
		return 0, clang.SysMenuTypeEmptyCode, lang.MsgErr(clang.SysMenuTypeEmptyCode, e.Lang)
	}
	if c.Sort < 0 {
		return 0, clang.SysMenuSortEmptyCode, lang.MsgErr(clang.SysMenuSortEmptyCode, e.Lang)
	}
	...
	if c.MenuType == constant.MenuM || c.MenuType == constant.MenuC {
		...
		if c.Path != "" {
			...
			if count > 0 {
				return 0, clang.SysMenuPathExistCode, lang.MsgErr(clang.SysMenuPathExistCode, e.Lang)
			}
		}
		data.Path = c.Path
...
func (e *SysMenu) Update(c *dto.SysMenuUpdateReq, p *middleware.DataPermission, cb *casbin.SyncedEnforcer) (bool, int, error) {
	...
	if c.Title == "" {
		return false, clang.SysMenuTitleEmptyCode, lang.MsgErr(clang.SysMenuTitleEmptyCode, e.Lang)
	}
	if c.MenuType == "" {
		return false, clang.SysMenuTypeEmptyCode, lang.MsgErr(clang.SysMenuTypeEmptyCode, e.Lang)
	}
	if c.Sort < 0 {
		return false, clang.SysMenuSortEmptyCode, lang.MsgErr(clang.SysMenuSortEmptyCode, e.Lang)
	}
	...
	if c.MenuType == constant.MenuM || c.MenuType == constant.MenuC {
		...
		if c.Path != "" && data.Path != c.Path {
			...
			if respCode == clang.SuccessCode && resp.Id != data.Id {
				return false, clang.SysMenuPathExistCode, lang.MsgErr(clang.SysMenuPathExistCode, e.Lang)
			}
		}
		if c.Path != "" && data.Path != c.Path {
...
func (e *SysMenu) Delete(ids []int64, p *middleware.DataPermission) (int, error) {
	...
	if count > 0 {
		return clang.SysMenuHasChildCode, lang.MsgErr(clang.SysMenuHasChildCode, e.Lang)
	}
...
func (e *SysMenu) getByRoleKey(roleKey string) ([]models.SysMenu, int, error) {
	...
	var err error
	if roleKey == global.RoleKeyAdmin {
		var data []models.SysMenu
	...
	return menuList, clang.SuccessCode, nil
}
