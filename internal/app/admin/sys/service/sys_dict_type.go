...
func (e *SysDictType) Insert(c *dto.SysDictTypeInsertReq) (int64, int, error) {
	...
	if c.DictName == "" {
		return 0, clang.SysDictTypeNameEmptyCode, lang.MsgErr(clang.SysDictTypeNameEmptyCode, e.Lang)
	}
	if c.DictType == "" {
	...
	if count > 0 {
		return 0, clang.SysDictTypeTypeExistCode, lang.MsgErr(clang.SysDictTypeTypeExistCode, e.Lang)
	}
...
func (e *SysDictType) Update(c *dto.SysDictTypeUpdateReq, p *middleware.DataPermission) (bool, int, error) {
	...
	if c.DictName == "" {
		return false, clang.SysDictTypeNameEmptyCode, lang.MsgErr(clang.SysDictTypeNameEmptyCode, e.Lang)
	}
	if c.DictType == "" {
		return false, clang.SysDictTypeTypeEmptyCode, lang.MsgErr(clang.SysDictTypeTypeEmptyCode, e.Lang)
	}
	...
	if c.DictType != "" && data.DictType != c.DictType {
		...
		if er != nil && respCode != clang.DataNotFoundCode {
			err = er
			return false, respCode, err
		}
		if respCode == clang.SuccessCode && resp.Id != data.Id {
			return false, clang.SysDictTypeTypeExistCode, lang.MsgErr(clang.SysDictTypeTypeExistCode, e.Lang)
		}
...
func (e *SysDictType) Delete(ids []int64, p *middleware.DataPermission) (int, error) {
	...
	for _, id := range ids {
		...
		if count > 0 {
			return clang.SysDictTypeTypeHasUsedCode, lang.MsgErr(clang.SysDictTypeTypeHasUsedCode, e.Lang)
		}
	}
