...
func (e *User) queryMaxTreeSort(queryCondition *dto.UserQueryReq) (int64, int, error) {
	...
	return maxSort, clang.SuccessCode, nil
}
...
func (e *User) Insert(c *dto.UserInsertReq) (int, error) {
	...
	if c.Emails == "" && c.Mobiles == "" {
		return clang.UserEmailOrMobileNeedCode, lang.MsgErr(clang.UserEmailOrMobileNeedCode, e.Lang)
	}
	...
	if c.Emails != "" {
		...
		for _, email := range emails {
			if !strutils.IsEmail(email) {
				return clang.UserEmailFormatErrCode, lang.MsgErr(clang.UserEmailFormatErrCode, e.Lang)
			}
		}
	}
	...
	if c.Mobiles != "" {
		if c.MobileTitle == "" {
			return clang.UserMobileNeedTitleCode, lang.MsgErr(clang.UserMobileNeedTitleCode, e.Lang)
		}
		mobiles = strings.Split(c.Mobiles, ",")
		for _, mobile := range mobiles {
			if !strutils.IsMobile(mobile) {
				return clang.UserMobileFormatErrCode, lang.MsgErr(clang.UserMobileFormatErrCode, e.Lang)
			}
		}
	}
	...
	for _, mobile := range mobiles {
		...
		if err != nil {
			return clang.UserMobileEncryptErrLogCode, lang.MsgLogErrf(e.Log, e.Lang, clang.UserMobileEncryptErrCode, clang.UserMobileEncryptErrLogCode, err)
		}
	...
	for _, email := range emails {
		...
		if err != nil {
			return clang.UserEmailEncryptErrLogCode, lang.MsgLogErrf(e.Log, e.Lang, clang.UserEmailEncryptErrCode, clang.UserEmailEncryptErrLogCode, err)
		}
...
func (e *User) register(registerType, email, mobile, mobileTitle string, refUser *models.User) (int, error) {
	...
	if count > 0 {
		...
		return clang.UserAccountExistLogCode, lang.MsgErrf(clang.UserAccountExistLogCode, e.Lang, account)
	}
...
func (e *User) insertMemUser(registerType, email, mobile, mobileTitle string, refUser *models.User) (int64, int, error) {
	...
	if count > 0 {
		return 0, clang.UserRefCodeErrLogCode, lang.MsgLogErrf(e.Log, e.Lang, clang.UserRegisterErrCode, clang.UserRefCodeErrLogCode, err)
	}
...
func (e *User) Update(c *dto.UserUpdateReq, p *middleware.DataPermission) (bool, int, error) {
	...
	if (c.Mobile != "" && c.MobileTitle == "") || (c.Mobile == "" && c.MobileTitle != "") {
		return false, clang.UserMobileNeedTitleCode, lang.MsgErr(clang.UserMobileNeedTitleCode, e.Lang)
	}
	...
	if c.Mobile != "" {
		...
		if err != nil {
			return false, clang.UserMobileEncryptErrLogCode, lang.MsgLogErrf(e.Log, e.Lang, clang.UserMobileEncryptErrCode, clang.UserMobileEncryptErrLogCode, err)
		}
	}
	...
	if c.Email != "" {
		...
		if err != nil {
			return false, clang.UserEmailEncryptErrLogCode, lang.MsgLogErrf(e.Log, e.Lang, clang.UserEmailEncryptErrCode, clang.UserEmailEncryptErrLogCode, err)
		}
	}
	...
	if mobile != "" {
		...
		if respCode == clang.SuccessCode && mobileUser.Id != c.Id && mobile == mobileUser.Mobile {
			return false, clang.UserMobileExistLogCode, lang.MsgErr(clang.UserMobileExistLogCode, e.Lang)
		}
	}
	...
	if email != "" {
		...
		if respCode == clang.SuccessCode && emailUserUser.Id != c.Id && email == emailUserUser.Email {
			return false, clang.UserEmailExistLogCode, lang.MsgErr(clang.UserEmailExistLogCode, e.Lang)
		}
	}
...
func (e *User) UpdateStatus(c *dto.UserStatusUpdateReq, p *middleware.DataPermission) (bool, int, error) {
	...
	if c.Status == "" {
		return false, clang.UserStatusEmptyCode, lang.MsgErr(clang.UserStatusEmptyCode, e.Lang)
	}
	var err error
...
func (e *User) GetSummaries(c *dto.UserQueryReq, p *middleware.DataPermission) (*models.User, int, error) {
	...
	result := models.User{}
	err = e.Orm.Model(&models.User{}).
		Select("sum(biz_user.money) as money").
		Scopes(
			cDto.MakeCondition(c.GetNeedSearch()),
			middleware.Permission(data.TableName(), p),
