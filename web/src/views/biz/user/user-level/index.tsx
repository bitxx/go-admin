...
import { delUserLevelApi, exportUserLevelApi, getUserLevelPageApi, UserLevelModel } from "@/api/biz/user/user-level";
...
const UserLevel: React.FC = () => {
	...
	const columns: ProColumns<UserLevelModel>[] = [
		...
		},
		{
			...
			render: (_, data) => (
				<Space>
					<HocAuth permission={["app:user-level:edit"]}>
						<LoadingButton
							key="edit"
							type="link"
							size="small"
					...
					<HocAuth permission={["app:user-level:del"]}>
						<LoadingButton
							key="delete"
							type="link"
							size="small"
	...
	useEffect(() => {
		const initData = async () => {
			const { data: levelTypeData, msg: levelTypeMsg, code: levelTypeCode } = await getDictsApi("biz_user_level_type");
	...
	const handleExport = (done: () => void) => {
		modal.confirm({
			...
			onOk: async () => {
				try {
					saveExcelBlob("用户等级", await exportUserLevelApi(tableFormRef.current?.getFieldsValue()));
				} catch (err) {
	...
	const handleDelete = (id: number, done: () => void) => {
		modal.confirm({
			...
			onOk: async () => {
				try {
					const { code, msg } = await delUserLevelApi([id!]);
