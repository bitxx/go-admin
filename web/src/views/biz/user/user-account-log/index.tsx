...
import { exportUserAccountLogApi, getUserAccountLogPageApi, UserAccountLogModel } from "@/api/biz/user/user-account-log";
...
const UserAccountLog: React.FC = () => {
	...
	useEffect(() => {
		const initData = async () => {
			const { data: moneyTypeData, msg: moneyTypeMsg, code: moneyTypeCode } = await getDictsApi("biz_money_type");
			if (moneyTypeCode !== ResultEnum.SUCCESS) {
				message.error(moneyTypeMsg);
				return;
			}
			setMoneyTypeOptions(getDictOptions(moneyTypeData));
			const { data: changeTypeData, msg: changeTypeMsg, code: changeTypeCode } = await getDictsApi("biz_account_change_type");
