...
import { exportUserOperLogApi, getUserOperLogPageApi, UserOperLogModel } from "@/api/biz/user/user-oper-log";
...
const UserOperLog: React.FC = () => {
	...
	useEffect(() => {
		const initData = async () => {
			const { data: actionTypeData, msg: actionTypeMsg, code: actionTypeCode } = await getDictsApi("biz_user_action_type");
			if (actionTypeCode !== ResultEnum.SUCCESS) {
				message.error(actionTypeMsg);
				return;
			}
			setActionTypeOptions(getDictOptions(actionTypeData));
			const { data: byTypeData, msg: byTypeMsg, code: byTypeCode } = await getDictsApi("biz_user_by_type");
