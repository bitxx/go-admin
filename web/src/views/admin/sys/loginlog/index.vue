    ...
    <template #wrapper>
      <el-card class="box-card">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              v-permisaction="['admin:sys-login-log:export']"
              type="success"
              icon="el-icon-plus"
              size="mini"
              @click="handleExport"
        ...
        <el-table v-loading="loading" stripe border :data="sysloginlogList">
          ...
          <el-table-column label="操作" fixed="right" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-button
                v-permisaction="['admin:sys-login-log:del']"
                size="mini"
                type="text"
                icon="el-icon-delete"
...
<script>
import { delSysLoginlog, listSysLoginlog, exportSysLoginlog } from '@/api/admin/sys/loginlog'
...
export default {
  ...
  created() {
    this.getList()
    this.getDicts('admin_sys_loginlog_status').then(response => {
