    ...
    <template #wrapper>
      <el-card class="box-card">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button
              v-permisaction="['admin:sys-config:add']"
              type="primary"
              icon="el-icon-plus"
              size="mini"
              @click="handleAdd"
          ...
          <el-col :span="1.5">
            <el-button
              v-permisaction="['content:sys-config:export']"
              type="warning"
              icon="el-icon-plus"
              size="mini"
              @click="handleExport"
        ...
        <el-table v-loading="loading" stripe border :data="configList">
          ...
          <el-table-column width="160" fixed="right" label="操作" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-button
                v-permisaction="['content:sys-config:edit']"
                size="mini"
                type="text"
                icon="el-icon-edit"
              ...
              <el-button
                v-permisaction="['content:sys-config:del']"
                size="mini"
                type="text"
                icon="el-icon-delete"
...
<script>
import { listConfig, getConfig, delConfig, addConfig, updateConfig, exportSetConfig } from '@/api/admin/sys/config'
...
export default {
  ...
  created() {
    this.getList()
    this.getDicts('admin_sys_config_type').then(response => {
      this.configTypeOptions = response.data
    })
    this.getDicts('admin_sys_config_is_frontend').then(response => {
