...
import { delPostApi, exportPostApi, getPostPageApi, PostModel } from "@/api/admin/sys/sys-post";
...
const Post: React.FC = () => {
    ...
    },
    {
      ...
      render: (_, data) => (
        <Space>
          ...
          <HocAuth permission={["admin:sys-post:del"]}>
            <LoadingButton
              key="delete"
              type="link"
              size="small"
  ...
  const handleExport = (done: () => void) => {
    modal.confirm({
      ...
      onOk: async () => {
        try {
          saveExcelBlob("Post", await exportPostApi(tableFormRef.current?.getFieldsValue()));
        } catch (err) {
  ...
  const handleDelete = (id: number, done: () => void) => {
    modal.confirm({
      ...
      onOk: async () => {
        try {
          const { code, msg } = await delPostApi([id!]);
    ...
    <>
      <ProTable<PostModel>
        ...
        request={async params => {
          const { data } = await getPostPageApi(params);
