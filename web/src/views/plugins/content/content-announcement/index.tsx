...
import {
  ...
  exportContentAnnouncementApi,
  getContentAnnouncementPageApi
...
const ContentAnnouncement: React.FC = () => {
  ...
  // 定义列
  const columns: ProColumns<ContentAnnouncementModel>[] = [
    {
      title: "序号",
      dataIndex: "index",
      valueType: "index",
    ...
    },
    {
      ...
      render: (_, data) => (
        <Space>
          <HocAuth permission={["plugins:content-announcement:edit"]}>
            <LoadingButton
              key="edit"
              type="link"
              size="small"
          ...
          <HocAuth permission={["plugins:content-announcement:del"]}>
            <LoadingButton
              key="delete"
              type="link"
              size="small"
  ...
  const handleExport = (done: () => void) => {
    modal.confirm({
      ...
      onOk: async () => {
        try {
          saveExcelBlob("公告管理", await exportContentAnnouncementApi(tableFormRef.current?.getFieldsValue()));
        } catch (err) {
  ...
  const handleDelete = (id: number, done: () => void) => {
    modal.confirm({
      ...
      onOk: async () => {
        try {
          const { code, msg } = await delContentAnnouncementApi([id!]);
  ...
  const toolBarRender = () => [
    <HocAuth permission={["plugins:content-announcement:add"]}>
      <LoadingButton type="primary" key="addTable" icon={<PlusCircleOutlined />} onClick={done => handleShowAddFormModal(done)}>
        新增
      </LoadingButton>
    </HocAuth>,
    <HocAuth permission={["plugins:content-announcement:export"]}>
  ...
  return (
    <>
      <ProTable<ContentAnnouncementModel>
        ...
        request={async params => {
          const { data } = await getContentAnnouncementPageApi(params);
