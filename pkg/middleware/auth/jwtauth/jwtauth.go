...
import (
	...
	"go-admin/pkg/config"
	"go-admin/pkg/dto/response"
	"go-admin/pkg/global"
	"go-admin/pkg/lang"
	"go-admin/pkg/middleware/auth/authdto"
	"go-admin/pkg/middleware/auth/casbin"
	...
	return func(c *gin.Context) {
		...
		if e == nil {
			...
			response.Error(c, http.StatusInternalServerError,
				lang.MsgByCode(http.StatusInternalServerError, lang.GetAcceptLanguage(c)))
			return
		}
		...
		if err != nil {
			...
			response.Error(c, http.StatusInternalServerError, lang.MsgByCode(http.StatusInternalServerError, lang.GetAcceptLanguage(c)))
			return
		}
		...
		if !res {
			...
			response.Error(c, http.StatusForbidden, lang.MsgByCode(http.StatusForbidden, lang.GetAcceptLanguage(c)))
			c.Abort()
			return
		}
...
func (j *JwtAuth) LoginResponse(c *gin.Context, token *core.Token) {
	...
	if err != nil {
		j.unauthorized(c, errCode, err.Error())
		return
	}
	...
	}, http.StatusOK, lang.MsgByCode(http.StatusOK, lg))
}
...
func (j *JwtAuth) RefreshResponse(c *gin.Context, token *core.Token) {
	...
	response.OK(c, gin.H{
		"token":  token.AccessToken,
		"expire": token.ExpiresAt,
	}, http.StatusOK, lang.MsgByCode(http.StatusOK, lang.GetAcceptLanguage(c)))
}
...
func (j *JwtAuth) revokeToken(c *gin.Context) (int, error) {
	...
	if err != nil {
		return http.StatusUnauthorized, lang.MsgErr(http.StatusUnauthorized, lang.GetAcceptLanguage(c))
	}
	userIDStr, errCode, err := j.GetUserIdStr(c)
	if err != nil {
		return errCode, err
...
func (j *JwtAuth) GetUserIdStr(c *gin.Context) (string, int, error) {
	...
	if userID <= 0 {
		return "", http.StatusUnauthorized, lang.MsgErrf(http.StatusUnauthorized, lang.GetAcceptLanguage(c))
	}
	return strconv.FormatInt(userID, 10), http.StatusOK, nil
}

func (j *JwtAuth) GetUserId(c *gin.Context) (int64, int, error) {
	userID := c.GetInt64(authdto.LoginUserId)
	if userID <= 0 {
		return 0, http.StatusUnauthorized, lang.MsgErrf(http.StatusUnauthorized, lang.GetAcceptLanguage(c))
	}
...
func (j *JwtAuth) Unauthorized(c *gin.Context, httpCode int, message string) {
	_, _ = j.revokeToken(c)
	log.GetRequestLogger(c).Errorf("Unauthorized failed: %d-%s", httpCode, message)
...
func (j *JwtAuth) unauthorized(c *gin.Context, httpCode int, message string) {
	...
	j.mw.Unauthorized(c, httpCode, message)
}
